...
export default function PresentationOutputWindow() {
  ...
  const attributionNode = !slideData.isBlank && slideData.title ? (
    <h2
    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 flex flex-col justify-between h-full">
          ...
          <div className="flex items-center justify-between gap-4 text-sm font-semibold tracking-widest text-[#D4A94A] uppercase border-b border-white/20 pb-4 drop-shadow">
            <div className="flex-1">{headerLeft}</div>
            {attributionPos === 'top' ? <div className="text-center">{attributionNode}</div> : null}
          ...
          {attributionPos !== 'top' && (
            <div className="relative flex items-center pt-6 border-t border-white/20 text-[#D4A94A]">
              <div className={`absolute left-0 drop-shadow ${attributionPos === 'bottom-right' ? 'invisible' : ''}`}>{footerLeft}</div>
              <div className={`w-full ${attributionPos === 'bottom-right' ? 'text-right' : 'text-center'}`}>
                {attributionNode}
              </div>
  ...
  if (isLight) {
    return (
      <div className={`h-screen w-screen bg-[#FFFFFF] text-[#111827] flex flex-col justify-between ${marginClass} select-none overflow-hidden font-sans`}>
        {/* Top Header Label */}
        ...
        {attributionPos !== 'top' && (
          <div className="relative flex items-center pt-6 border-t border-[#E5E7EB] text-[#B4821E]">
            <div className={`absolute left-0 ${attributionPos === 'bottom-right' ? 'invisible' : ''}`}>{footerLeft}</div>
            <div className={`w-full ${attributionPos === 'bottom-right' ? 'text-right' : 'text-center'}`}>
              {attributionNode}
            </div>
  ...
  // Dark Theme (Default)
  return (
    <div className={`h-screen w-screen bg-[#0B0C0E] text-[#EDEDEE] flex flex-col justify-between ${marginClass} select-none overflow-hidden font-sans`}>
      {/* Top Header Label */}
      <div className="flex items-center justify-between gap-4 text-sm font-semibold tracking-widest text-[#D4A94A] uppercase border-b border-[#2A2C31]/40 pb-4">
        <div className="flex-1">{headerLeft}</div>
        {attributionPos === 'top' ? <div className="text-center">{attributionNode}</div> : null}
        <div className="flex-1 flex justify-end">{headerRight}</div>
      </div>
      ...
      {/* Reference / Attribution Line Below & Footer App Name */}
      {attributionPos !== 'top' && (
        <div className="relative flex items-center pt-6 border-t border-[#2A2C31]/40 text-[#D4A94A]">
          <div className={`absolute left-0 ${attributionPos === 'bottom-right' ? 'invisible' : ''}`}>{footerLeft}</div>
          <div className={`w-full ${attributionPos === 'bottom-right' ? 'text-right' : 'text-center'}`}>
            {attributionNode}
          </div>
