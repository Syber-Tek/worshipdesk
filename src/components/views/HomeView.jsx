...
export default function HomeView({
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-6">
      ...
      <div
        className={`p-5 rounded-xl border flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-colors duration-200 ${cardClass}`}
      >
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="px-3.5 py-2 bg-accent hover:bg-accent-hover text-bg font-bold text-xs rounded-lg flex items-center gap-1.5 shadow transition cursor-pointer"
          >
          ...
          <button
            ...
            className={`px-3.5 py-2 border rounded-lg font-semibold text-xs flex items-center gap-1.5 transition cursor-pointer ${
              isLight
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={`p-4 rounded-xl border space-y-3 ${cardClass}`}>
          ...
          <div>
            ...
            >
              {playlist.length}{" "}
              <span className="text-xs font-normal opacity-70">Items</span>
            </div>
            <p className={`text-[11px] mt-0.5 ${labelClass}`}>
              {activePlaylistItem
                ? `Live: ${activePlaylistItem.title}`
                : "No item live"}
            </p>
          </div>
          <div className="pt-2 border-t border-border flex justify-between items-center text-[10px]">
            <span className="text-success font-medium flex items-center gap-1">
              <Activity set="bold" primaryColor="#6FCF97" size="small" /> Ready
              for Live
            </span>
            <button
              onClick={() => setActiveTab("plan")}
            ...
            >
              View Plan{" "}
              <ChevronRight set="light" primaryColor="#D4A94A" size="small" />
            </button>
          </div>
        </div>
        ...
        <div className={`p-4 rounded-xl border space-y-3 ${cardClass}`}>
          <div className="flex justify-between items-center">
            ...
            <Video
              set="bold"
              primaryColor={displays.length > 1 ? "#6FCF97" : "#D4A94A"}
              size="small"
            />
          </div>
          <div>
            ...
            >
              {displays.length}{" "}
              <span className="text-xs font-normal opacity-70">
                {displays.length > 1 ? "Monitors" : "Monitor"}
              </span>
            </div>
            <p className={`text-[11px] mt-0.5 ${labelClass}`}>
              {displays.length > 1
                ? "Dual Display Mode Active"
          ...
          <div className="pt-2 border-t border-border flex justify-between items-center text-[10px]">
            <span
              className={
                displays.length > 1 ? "text-success font-medium" : labelClass
              }
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className={`p-4 rounded-xl border space-y-3 ${cardClass}`}>
            ...
            <div className="relative flex items-center">
              <div className="absolute left-3 pointer-events-none">
                <Search
                  set="light"
                  primaryColor={isLight ? "#6B7280" : "#696C75"}
                  size="small"
                />
              </div>
              <input
              ...
              <button
                ...
                className="absolute right-1.5 px-3 py-1.5 bg-accent text-bg font-bold text-xs rounded transition hover:bg-accent-hover cursor-pointer"
              >
                Go ➔
              </button>
            </div>
          ...
          <div className={`p-5 rounded-xl border space-y-4 ${cardClass}`}>
            <div className="flex justify-between items-center border-b pb-3 border-border">
              <div>
                ...
                <p className={`text-[11px] ${labelClass}`}>
                  Quick view of prepared order of worship
                </p>
              </div>
              <button
                onClick={() => setActiveTab("plan")}
            ...
            {playlist.length === 0 ? (
              <div
                className={`p-6 rounded-lg border text-center space-y-2 ${innerCardClass}`}
              >
                <InfoSquare
                  set="light"
                  ...
                  size="medium"
                />
                <p className={`text-xs font-medium ${headingClass}`}>
                  No items in playlist
                </p>
                <p className={`text-[11px] ${labelClass}`}>
                  Add verses, hymns, or sermon slides to construct your worship
                  service.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p
                          className={`text-[11px] truncate mt-0.5 ${labelClass}`}
                        >
                          {item.content}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        title="Stage as Next"
                        className="p-1.5 border border-border rounded hover:border-accent text-accent transition cursor-pointer"
                      >
                        <Show set="bold" primaryColor="#D4A94A" size="small" />
                      </button>
                      <button
                        onClick={() =>
                          handlePresentNow && handlePresentNow(item)
                        }
                        title="Present Live"
                        className="p-1.5 bg-accent text-bg rounded font-bold hover:bg-accent-hover transition shadow cursor-pointer"
                      >
        ...
        <div className="space-y-5">
          ...
          <div className={`p-4 rounded-xl border space-y-3.5 ${cardClass}`}>
            <div className="flex justify-between items-center border-b pb-2.5 border-border">
              <span className="text-[10px] font-bold uppercase tracking-wider text-live flex items-center gap-1">
            ...
            >
              {currentSlide ? (
                <div>
                  ...
                  <p
                    className={`text-[11px] line-clamp-3 leading-relaxed mt-1 ${labelClass}`}
                  >
                    "{currentSlide.content || currentSlide.text}"
                  </p>
                </div>
              ) : (
                <p className={`text-xs italic text-center ${labelClass}`}>
                  No slide active on screen
                </p>
              )}
            </div>
