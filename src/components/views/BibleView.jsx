...
export default function BibleView({
  ...
  const translations = biblesList && biblesList.length > 0
  ...
  const dbLists = {
    ot: allBooksList && allBooksList.length > 0
      ? allBooksList.filter(b => (b.book_number || 0) <= 39)
      : [],
    nt: allBooksList && allBooksList.length > 0
      ? allBooksList.filter(b => (b.book_number || 0) > 39)
      : []
  }
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      ...
      <div className={`flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-lg border transition-colors duration-200 ${
        isLight
          ? 'bg-[#FFFFFF] border-[#E5E7EB]'
          : 'bg-[#151619] border-[#2A2C31]'
      }`}>
        <div className="flex items-center gap-3">
          <span className={`text-xs font-bold uppercase tracking-wider ${isLight ? 'text-[#6B7280]' : 'text-[#6B6C73]'}`}>
            Translation:
          </span>
      ...
      <div className={`flex items-center justify-between px-4 py-2 rounded-lg border text-xs font-medium ${
                ...
                {otBooks.map((b) => (
                  <div key={b.name} className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {Array.from({ length: b.chaptersCount || 1 }, (_, i) => i + 1).map((ch) => {
                        const isSelected = normalizeBookName(selectedBook) === normalizeBookName(b.name) && selectedChapter === ch
                        return (
                          <button
                            key={ch}
                            onClick={() => {
      ...
      {viewMode === 'reader' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          <div className={`lg:col-span-7 border rounded-lg p-3 space-y-2 max-h-125 overflow-y-auto ${
            isLight ? 'bg-[#FFFFFF] border-[#E5E7EB]' : 'bg-[#151619] border-[#2A2C31]'
          }`}>
          ...
          <div className={`lg:col-span-5 border rounded-lg p-5 flex flex-col justify-between ${
            isLight ? 'bg-[#FFFFFF] border-[#E5E7EB]' : 'bg-[#151619] border-[#2A2C31]'
          }`}>
