...
export default function Header({ displays = [], isLive, setIsLive, broadcastToPresentation, themeMode, effectiveTheme, setThemeMode }) {
  const toggleLiveStatus = () => {
    ...
    if (nextLive) {
      appNotify({ title: 'LIVE ON-AIR', body: 'Presentation output is now broadcasting live.', type: 'success' })
    } else {
      appNotify({ title: 'STANDBY MODE', body: 'Presentation output is in standby mode.', type: 'info' })
    }
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <span className={`hidden sm:inline ${isLight ? 'text-[#D1D5DB]' : 'text-[#6B6C73]'}`}>|</span>
        <div className={`hidden sm:flex items-center gap-1.5 text-[11px] ${isLight ? 'text-[#4B5563]' : 'text-[#9B9CA3]'}`}>
