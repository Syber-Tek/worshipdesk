...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window !== 'undefined' && window.localStorage) {
      ...
      if (saved && ['home', 'plan', 'bible', 'songs', 'settings'].includes(saved)) return saved
    }
    return 'home'
  })
  ...
  const [outputFontSize, setOutputFontSize] = useState(() => {
    return localStorage.getItem('church_presenter_output_font_size') || 'normal'
  })
  ...
  const [projectionDisplays, setProjectionDisplays] = useState(() => {
    if (typeof window !== 'undefined' && window.localStorage) {
      try {
        const saved = JSON.parse(localStorage.getItem('church_presenter_projection_displays') || '[]')
  ...
  useEffect(() => {
    localStorage.setItem('church_presenter_attribution_pos', attributionPosition)
  }, [attributionPosition])
  ...
  useEffect(() => {
    localStorage.setItem('church_presenter_default_hymn_cat', defaultHymnCategory)
  }, [defaultHymnCategory])

  useEffect(() => {
    localStorage.setItem('church_presenter_show_hymn_nums', showHymnNumbers ? 'true' : 'false')
  }, [showHymnNumbers])
  ...
  useEffect(() => {
    localStorage.setItem('church_presenter_projection_displays', JSON.stringify(projectionDisplays))
  }, [projectionDisplays])
...
// so an empty search result correctly shows nothing instead of placeholder data.
  ...
  }, [currentSlide, isLive, isBlank, isBlack, outputTheme, outputBgImage, showVerseQuotes, appNamePosition, customHeaderTitle, slideMargin, attributionPosition, outputFontSize, selectedVerseIndex, filteredVerses.length, hymnDeck, hymnDeckIndex])
  ...
  const normalizeSlide = (item) => {
    ...
    return {
      title: isVerse ? `${item.ref} (${selectedTranslation})` : item.title || 'Untitled',
  ...
  const handleAddToPlaylist = (item) => {
    ...
    setPlaylist((prev) => [...prev, { ...slide, id: `item-${Date.now()}`, status: 'pending' }])
  }
  ...
  return (
    <div
      ...
      style={{
        fontSize:
          uiScale === 'compact' ? '12px' : uiScale === 'large' ? '14px' : '13px'
      }}
