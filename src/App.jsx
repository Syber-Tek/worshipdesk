...
export default function App() {
  ...
  const [customHeaderTitle, setCustomHeaderTitle] = useState(() => {
    return (
      localStorage.getItem("church_presenter_header_title") ||
      "Church Presenter"
    );
  });
  ...
  const [presentationFontSize, setPresentationFontSize] = useState(() => {
    return (
      localStorage.getItem("church_presenter_presentation_font_size") || "normal"
    );
  });
  ...
  const [attributionPos, setAttributionPos] = useState(() => {
  ...
  useEffect(() => {
    localStorage.setItem(
      "church_presenter_presentation_font_size",
  ...
  useEffect(() => {
    localStorage.setItem("church_presenter_attribution_pos", attributionPos);
  }, [attributionPos]);
  ...
  const [projectionDisplays, setProjectionDisplays] = useState(null); // display ids to project to (null = not yet initialized)
  ...
  // Fetch verses dynamically from SQLite or the XML
  ...
  // Projection display selection: initialize with the saved choice (falling back to
  // the secondary display, or the primary when that is the only screen) and keep the
  // projector windows in sync when displays are added/removed.
  useEffect(() => {
    if (window.api && window.api.openPresentationWindows) {
      const syncProjection = (ids) =>
        window.api.openPresentationWindows(ids.filter((id) => displays.some((d) => d.id === id)));

      if (!displays.length) return;

      if (projectionDisplays === null) {
        let saved = null;
        try {
          saved = JSON.parse(localStorage.getItem("cp_projection_ids"));
        } catch {
          saved = null;
        }
        const stored = Array.isArray(saved)
          ? saved.filter((id) => displays.some((d) => d.id === id))
          : null;
        const hasStored = stored !== null && stored.length > 0;
        let init = stored || [];
        if (!hasStored) {
          const secondary = displays.find((d) => !d.isPrimary);
          init = secondary ? [secondary.id] : displays[0] ? [displays[0].id] : [];
        }
        setProjectionDisplays(init);
        localStorage.setItem("cp_projection_ids", JSON.stringify(init));
        if (window.api.openPresentationWindows) window.api.openPresentationWindows(init);
      } else {
        const valid = projectionDisplays.filter((id) => displays.some((d) => d.id === id));
        if (valid.join(",") !== projectionDisplays.join(",")) {
          setProjectionDisplays(valid);
          localStorage.setItem("cp_projection_ids", JSON.stringify(valid));
          syncProjection(valid);
        }
      }
    }
  ...
  const updateProjectionDisplays = (ids) => {
    const cleaned = ids.filter((id) => displays.some((d) => d.id === id));
    setProjectionDisplays(cleaned);
    localStorage.setItem("cp_projection_ids", JSON.stringify(cleaned));
    if (window.api && window.api.openPresentationWindows) {
      window.api.openPresentationWindows(cleaned);
    }
  };

  const onToggleProjectionDisplay = (id) => {
    const current = Array.isArray(projectionDisplays) ? projectionDisplays : [];
    const next = current.includes(id) ? current.filter((x) => x !== id) : [...current, id];
    updateProjectionDisplays(next);
  };
        ...
        <main className="flex-1 overflow-y-auto p-5 text-xs">
          ...
          {activeTab === "settings" && (
            <SettingsView
              ...
              attributionPos={attributionPos}
              setAttributionPos={setAttributionPos}
            />
          )}
        </main>
      </div>
